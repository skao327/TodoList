...
class Todo:
    ...
    def delete_task(self):
        ...
        if selected:
            ...
            try:
                # 삭제 전 항목 임시저장 (복구용)
                self.cursor.execute("SELECT id, task, completed FROM todolists WHERE id=%s", (task_id,))
    ...
    def undo_delete(self):
        if self.deleted_item_stack:
            ...
            try:
                self.cursor.execute("INSERT INTO todolists (task, completed) VALUES (%s, %s)", (task, completed))
                self.conn.commit()
                self.display_tasks()
            except mysql.connector.Error as e:
